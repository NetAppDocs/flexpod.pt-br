...
image:medical-imaging_image3.png["Um diagrama que mostra a infraestrutura de imagens médicas do FlexPod com UCS, Nexus, AFF e switches MDS"]
...
image:medical-imaging_image4.png["Um diagrama que mostra servidores SnapCenter, integração com VMware, repositórios de metadados replicados e plug-ins de aplicativos"]

NetApp MetroCluster configurações são usadas por milhares de empresas em todo o mundo para alta disponibilidade (HA), zero perda de dados e operações não disruptivas, tanto dentro quanto fora do data center. MetroCluster é um recurso gratuito do ONTAP que espelha de forma síncrona dados e configurações entre dois clusters ONTAP em locais separados ou domínios de falha. MetroCluster fornece armazenamento continuamente disponível para aplicações ao gerenciar automaticamente dois objetivos: zero ponto de recuperação (RPO), espelhando de forma síncrona os dados gravados no cluster. Objetivo de tempo de recuperação (RTO) próximo de zero, espelhando a configuração e automatizando o acesso aos dados no segundo site. MetroCluster oferece simplicidade com espelhamento automático de dados e configurações entre os dois clusters independentes localizados nos dois sites. À medida que o armazenamento é provisionado em um cluster, ele é automaticamente espelhado para o segundo cluster no segundo site. NetApp SyncMirror tecnologia fornece uma cópia completa de todos os dados com RPO zero. Portanto, as cargas de trabalho de um site podem ser transferidas a qualquer momento para o site oposto e continuar fornecendo dados sem perda de dados. Mais informações podem ser encontradas  https://fieldportal.netapp.com/s/article/tr4705-netapp-metrocluster-solution-architecture-and-design-1693982882873["aqui"^].
...
image:medical-imaging_image5.png["Um diagrama que mostra uma clínica comunitária conectada aos sistemas hospitalares por meio de um barramento de serviços corporativos"]
...
Os sistemas de imagem médica estão começando a usar  https://www.netapp.com/pdf.html?item=/media/7398-sb-flexpod-datacenter-ai.pdf["Recursos de computação baseados em GPU"^] para aprimorar os fluxos de trabalho humanos, pré-processando as imagens e, assim, aumentando a eficiência. Os sistemas de imagem médica corporativos típicos aproveitam as capacidades de eficiência de storage líderes do setor da NetApp. Os sistemas de imagem médica corporativos normalmente usam o RMAN para atividades de backup, recuperação e restauração. Para melhor desempenho e para reduzir o tempo necessário para criar backups, a tecnologia Snapshot está disponível para operações de backup e a tecnologia SnapMirror está disponível para replicação.

A figura abaixo mostra os componentes lógicos da aplicação numa vista arquitetónica em camadas.

image:medical-imaging_image6.png["Um diagrama que mostra componentes de imagem médica em camadas no FlexPod com recuperação de desastres replicada"]

A figura abaixo mostra os componentes físicos da aplicação.

image:medical-imaging_image7.png["Um diagrama que mostra data centers FlexPod ativos e em espera com armazenamento NetApp ONTAP e servidores de imagens médicas"]
