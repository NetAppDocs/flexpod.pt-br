...
* Standalone Cisco UCS Fabric Interconnect appliance ports são recomendadas apenas para conexões iSCSI. Quando conexões NAS são usadas com portas de appliance, switches Nexus / compatíveis adicionais são necessários para determinados cenários de falha.
* Nenhum equipamento adicional pode ser colocado em linha entre os componentes principais do FlexPod.


Ligações uplink:

* As portas nos controladores de armazenamento NetApp devem ser conectadas aos switches Cisco Nexus 9000 Series para habilitar o suporte a canais de porta virtuais (vPCs).
