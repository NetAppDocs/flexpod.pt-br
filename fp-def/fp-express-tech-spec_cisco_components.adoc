...
Os blades Cisco UCS B-Series atualmente suportados na plataforma Cisco UCS Mini são B200 M5 e B420 M4. Outros blades serão listados na tabela a seguir à medida que se tornarem suportados na plataforma Cisco UCS Mini.
...
Os blades Cisco UCS C-Series estão disponíveis em versões de uma e duas unidades de rack (RU), com diversas opções de CPU, memória e E/S. Os números de peça listados na tabela a seguir referem-se ao servidor de base; eles não incluem CPUs, memória, unidades de disco, placas PCIe ou o Cisco FEX. Múltiplas opções de configuração estão disponíveis e são suportadas em FlexPod.
...
A arquitetura FlexPod Express com NetApp AFF ou FAS é construída com o switch Cisco Nexus 31108. FlexPod Express com a arquitetura Cisco UCS Mini (gerenciada pelo Cisco UCS) é validada usando o switch Cisco Nexus 3524. Essa configuração também pode ser implementada com um switch padrão.

O FlexPod Express com E-Series pode ser implementado com um switch padrão.

A tabela a seguir lista os números de peça para os chassis Cisco Nexus series; eles não incluem módulos SFP adicionais ou módulos complementares.
...
Contratos de suporte SMARTnet válidos são obrigatórios para todos os equipamentos Cisco na arquitetura FlexPod Express.


NOTE: As licenças necessárias e os números de peça dessas licenças devem ser verificados pelo seu representante de vendas porque podem diferir para diferentes produtos.
