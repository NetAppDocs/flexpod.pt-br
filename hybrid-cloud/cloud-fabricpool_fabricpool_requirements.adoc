...
[role="lead"]
FabricPool requer ONTAP 9.2 ou posterior. Requisitos adicionais do FabricPool dependem da versão do ONTAP em uso e da camada de nuvem conectada. Em versões anteriores ao ONTAP 9.8, FabricPool é suportado apenas em camadas locais SSD. O suporte para agregados HDD foi adicionado no ONTAP 9.8. Para plataformas AFF de nível básico que possuem capacidade fixa e relativamente pequena, como o NetApp AFF C190, FabricPool pode ser altamente eficaz para mover dados inativos para a camada de nuvem.
...
* NetApp AFF
* NetApp FAS FabricPool é compatível com todas as plataformas capazes de executar ONTAP 9.2, exceto as seguintes:
+
** FAS8020
...
NOTE: Consulte o  https://hwu.netapp.com/DetailViews/Home.aspx["NetApp Hardware Universe"^] para obter as informações mais recentes sobre as plataformas compatíveis com FabricPool.

* NetApp ONTAP Select
+

NOTE: NetApp recomenda o uso de camadas locais FabricPool compostas exclusivamente por SSDs.
...
** NetApp StorageGRID 10.3 e versões posteriores
+

NOTE: Glacier Flexible Retrieval e Glacier Deep Archive não são suportados.
...
** Azure NetApp Files
** NetApp Cloud Volumes para Google Cloud
** NetApp Cloud Volumes ONTAP para AWS
** NetApp Cloud Volumes ONTAP para Azure
...
image:cloud-fabricpool_image6.png["Um diagrama que mostra dois controladores NetApp AFF A300 conectando LIFs intercluster ao Amazon S3"]
...
image:cloud-fabricpool_image7.png["Uma captura de tela dos resultados do profiler de armazenamento de objetos da CLI do ONTAP, mostrando a latência e a taxa de transferência das operações PUT e GET"]
...
FabricPool licenças (incluindo capacidade adicional para licenças existentes) podem ser adquiridas em incrementos de 1TB.
