...
summary: Esta seção descreve como instalar e configurar o Cloud Volumes ONTAP, o FlexPod, o armazenamento ONTAP e o NetApp Console. 
---
...
. Acesse o NetApp Console.
+
Para acessar o Console e outros serviços em nuvem, você precisa se cadastrar em https://cloudmanager.netapp.com/["NetApp Console"^] .  Para configurar espaços de trabalho e usuários na conta do Console, consulte https://docs.netapp.com/us-en/console-setup-admin/index.html["Configuração e administração do NetApp Console"^] .  Você precisa de uma conta que tenha permissão para implantar o agente do Console no seu provedor de nuvem diretamente do Console.  Para obter as permissões necessárias, consulte https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html["Resumo de permissões para o NetApp Console"^] .

. Implantar agente do console.
+
Antes de adicionar um sistema Cloud Volume ONTAP , você deve implantar um agente de console.  O Console avisa se você tentar criar seu primeiro sistema Cloud Volumes ONTAP sem um agente do Console instalado.  Para implantar um agente do Console na AWS a partir do Console, consulte o https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-creating-connectors-aws.html["Opções de instalação do agente de console na AWS"^] .
...
+
Nessa solução, implantamos um sistema Cloud Volumes ONTAP de nó único na AWS.
...
Para implantar a instância local do FlexPod, consulte  https://www.cisco.com/c/en/us/td/docs/unified_computing/ucs/UCS_CVDs/flexpod_xseries_vmware_7u2.html["este guia de implantação"^].
...
== Adicione armazenamento FlexPod local ao NetApp Console

Siga os passos abaixo para adicionar seu armazenamento FlexPod ao sistema usando o Console.

. No menu de navegação, selecione *Armazenamento* > *Sistemas*.
. Na página Sistemas, clique em *Adicionar sistema* e selecione *Local*.
...
+
O Console descobre o cluster ONTAP e o adiciona como um sistema na página Sistemas.
