...
....
+
Uma atualização do SnapMirror também pode ser realizada através da GUI do NetApp Console na guia *Replicação*.
...
+
Após a SVM ser interrompida, o `hc_iscsi_vol` O volume não está visível no console.
